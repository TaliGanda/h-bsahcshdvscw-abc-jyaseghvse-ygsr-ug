...
if (isNaN(ratelimit) || ratelimit <= 0) {
 ...
 if (enabled('uam')) {
    // Perbaikan: tingkatkan logika token UAM dengan pola yang lebih kompleks dan header tambahan
    function generateUamToken() {
        const part1 = randstrr(43);
        const part2 = randstrr(12);
        const part3 = timestampString;
        const part4 = '0-gaNy' + randstrr(8);
        return `${part1}_${part2}-${part3}-${part4}`;
    }

    hcookie = `cf_chl_rt_tk=${generateUamToken()}`;

    // Tambahkan header tambahan untuk meniru browser asli
    if (!customHeaders) {
        customHeaders = '';
    }
    const uamHeaders = [
    ...
    customHeaders += (customHeaders.length > 0 ? '#' : '') + uamHeaders.join('#');
}
...
const DYNAMIC_SETTINGS = {
    headerTableSize: [512, 1024, 2048, 4096, 8192, 12288, 16384, 24576, 32768, 49152, 65536, 98304, 131072, 196608, 262144],
    initialWindowSize: [32768, 65535, 131072, 262144, 393216, 524288, 786432, 1048576, 1572864, 2097152, 3145728, 4194304, 6291456, 8388608, 12582912],
...
// 6. Enhanced Frame Generation
function generateAdvancedFrames(streamId) {
    const frames = [];
    
    // Tambahkan frame PRIORITY 30% dari waktu
    if(Math.random() < 0.3) {
        frames.push(createPriorityFrame(streamId));
    }
    
    // Tambahkan CONTINUATION frame 40% dari waktu
    if(Math.random() < 0.4) {
        const continuationPayload = Buffer.from([0x00, 0x00]);
        frames.push(createContinuationFrame(streamId, continuationPayload));
    }
...
function createContinuationFrame(streamId, payload, flags = 0) {
    return encodeFrame(streamId, 0x9, payload, flags);
}

function createPriorityFrame(streamId) {
    const data = Buffer.alloc(5);
    data.writeUInt32BE(0, 0); // Stream dependency
    data.writeUInt8(255, 4);  // Weight
    return encodeFrame(streamId, 0x2, data);
}
...
function buildRequest() {
    ...
    let headersArray = [
        ...
        'Pragma: no-cache',
        'Expires: 0',
        `x-webgl-renderer: ${randstrr(12)}`,
        `x-webgl-vendor: ${randstrr(8)}`,
        `x-webgl-version: WebGL 2.0`,
    ...
    const netSocket = net.connect(Number(proxyPort), proxyHost, () => {
        netSocket.once('data', () => {
            tlsSocket = tls.connect({
                ...
                secureOptions: crypto.constants.SSL_OP_NO_RENEGOTIATION | crypto.constants.SSL_OP_NO_TICKET | crypto.constants.SSL_OP_PKCS1_CHECK_1 | crypto.constants.SSL_OP_PKCS1_CHECK_2 | crypto.constants.SSL_OP_NO_SSLv2 | crypto.constants.SSL_OP_NO_SSLv1 | crypto.constants.SSL_OP_NO_SSLv3 | crypto.constants.SSL_OP_NO_COMPRESSION | crypto.constants.SSL_OP_TLSEXT_PADDING | crypto.constants.SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION | crypto.constants.SSL_OP_CIPHER_SERVER_PREFERENCE | crypto.constants.SSL_OP_NO_TLSv1 | crypto.constants.SSL_OP_NO_TLSv1_1 | crypto.constants.SSL_OP_ALL | crypto.constants.SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION | crypto.constants.SSL_OP_CIPHER_SERVER_PREFERENCE |crypto.constants.SSL_OP_ENABLE_KTLS,
                secure: true,
                minVersion: 'TLSv1.2',
                maxVersion: 'TLSv1.3',
                rejectUnauthorized: false,
...
if (cluster.isMaster) {
    ...
    setInterval(TCP_CHANGES_SERVER, 2500);
